...
const { data: lifeCycle } = await useLazyAsyncData(
    `lifecycle-${lifecycleId}`,
    () => lifecycleService.getLifecycleById(lifecycleId),
    {
        server: false,
        watch: [() => lifecycleId]
    }
...
const { data: indices } = await useLazyAsyncData(
    `indices-${lifecycleId}`,
    async () => {
        const indicesArr = [defaultIndice];
        if (!lifeCycle.value?.Phases?.length) return indicesArr;
    ...
    {
        server: false,
        default: () => [defaultIndice],
        watch: [() => lifeCycle.value?.Phases]
    }
);

const { data: reflectionAnswers } = await useLazyAsyncData(
    `reflectionAnswers-${lifecycleId}-${auth.token}`,
    async () => {
        if (!lifeCycle.value?.Phases?.length || !auth.token) return [];
        reflectionAnswerService.setToken(auth.token);
        const arr: (ReflectionAnswer | undefined)[] = [];
        for (const phase of lifeCycle.value.Phases) {
            if (phase.Reflection) {
                arr.push(await reflectionAnswerService.GetReflectionAnswerByUserIdAndReflectionID(phase.Reflection.id));
            } else {
                arr.push(undefined);
            }
        }
    ...
    {
        server: false,
        default: () => [] as (ReflectionAnswer | undefined)[],
        watch: [() => lifeCycle.value?.Phases]
    }
);

const { data: journalAnswers } = await useLazyAsyncData(
    `journalAnswers-${lifecycleId}-${auth.token}`,
    async () => {
        if (!lifeCycle.value?.Phases?.length || !auth.token) return [];
        journalAnswerService.setToken(auth.token);
        const arr: (JournalAnswer | undefined)[] = [];
        for (const phase of lifeCycle.value.Phases) {
            if (phase.Journal) {
                arr.push(await journalAnswerService.GetJournalAnswerByUserIdAndJournalID(phase.Journal.id));
            } else {
                arr.push(undefined);
            }
        }
...
const { data: recommendations } = await useLazyAsyncData(
    `recommendations-${lifecycleId}-${reflectionAnswers.value}`,
    async () => {
        if (!lifeCycle.value?.Phases?.length || !auth.token) return [];
        const arr: Recommendation[][] = [];
        for (let i = 0; i < lifeCycle.value.Phases.length; i++) {
            const phase = lifeCycle.value.Phases[i];
            if (phase.Reflection && reflectionAnswers.value?.[i]) {
                arr.push(await recommendationService.getRecommendations(phase.Reflection.id, reflectionAnswers.value[i]!.binaryEvaluation));
            } else {
                arr.push([]);
            }
        }
    ...
    {
        server: false,
        default: () => [] as Recommendation[][],
        watch: [() => reflectionAnswers.value]
    }
);

const { data: recommendationAnswers } = await useLazyAsyncData(
    `recommendationAnswers-${lifecycleId}-${auth.token}`,
    async () => {
        if (!lifeCycle.value?.Phases?.length || !auth.token || !recommendations.value?.length) return [];
        recommendationAnswerService.setToken(auth.token);
        const arr: RecommendationAnswer[][] = [];
        for (let i = 0; i < recommendations.value.length; i++) {
            const phaseRecs = recommendations.value[i];
            const phaseAnswers: RecommendationAnswer[] = [];
            for (const rec of phaseRecs) {
                phaseAnswers.push(await recommendationAnswerService.GetRecommendationAnswerByUserIdAndRecommendationID(rec.id));
            }
            arr.push(phaseAnswers);
        }
        return arr;
    },
    {
        server: false,
        default: () => [] as RecommendationAnswer[][],
...
watch(
    ...
    (newIndices) => {
        ...
        let found = null;
        for (const group of newIndices) {
            found = group.children?.find(x => x.value === hash);
            if (found) break;
        }
        activeIndex.value = found || newIndices[0].children[0];
    },
    { immediate: true }
);
